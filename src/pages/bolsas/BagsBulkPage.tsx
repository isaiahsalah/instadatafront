...
import {getBagsEmbultaje} from "@/services/bolsas.api";
import {IBagsEmbultaje} from "@/utils/interfaces";
import {ColumnDef} from "@tanstack/react-table";
import {useContext, useEffect, useMemo, useState} from "react";

const BagsBulkPage = () => {
  const {dateRange} = useContext(DateRangeContext);
  const [printing, setPrinting] = useState<IBagsEmbultaje[] | null>(null);
  ...
  const updateView = async () => {
    const printingData: IBagsEmbultaje[] = await getBagsEmbultaje({
  ...
  const columns: ColumnDef<IBagsEmbultaje>[] = useMemo(() => {
    if (!printing) return [];
    return [
      {
        accessorKey: "turn",
